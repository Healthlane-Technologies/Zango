...
The migration process is similar to Django and consists of two main steps: `ws_makemigrate` and `ws_migrate`.

### Step 1: Generate Migrations with `ws_makemigrate`

The `ws_makemigrate` command is used to create migration files for your models. These migration files capture all changes made to your model definitions, such as adding new fields, modifying existing ones, or establishing new relationships. To generate migrations
