...
various applications on the Zango platform. The packages required for our todo app are [crud](../../packages/crud/overview), 
[frame](../../packages/frame/overview), [login](../../packages/basic-auth/overview) and [workflow](../../packages/workflow/overview).
...
and then clicking on the install package button. You can read more about Zango packages [here](../../category/packages).
...
[here](../../core/category/packages-ecosystem).
...
The Workflow component, offered by the [Workflow Package](../../category/workflow), is an essential tool for managing and structuring 
...
of the [Crud Package](../../category/workflow). We have to simply tell `BaseCrudView` about the Format to display the data in,
...
provided by the [frames package](../../packages/crud/overview).
In our todo app, we will allow our `Todo_user` role to access the `/todo/all` url.
![Todo app frames](/img/todoapp_frames.png)

#### Login
In the last step, we have to configure the [login package](../../packages/basic-auth/overview) to redirect us our todo app url
