...
def generate_lockout_response(request, credentials):
    ...
    if connection.tenant.tenant_type != "shared":
        return render(request, "accesslogs/account_lockout.html", {"logout_url": "/logout", "cooloff_time": cooloff_time})
    return render(request, "accesslogs/account_lockout.html", {"logout_url": "/auth/logout", "cooloff_time": cooloff_time})