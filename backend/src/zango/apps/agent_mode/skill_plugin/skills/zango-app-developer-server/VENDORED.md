...
| **SKILL.md had grown to 1005 lines**, 656 of them STEP 5 — the references exist precisely so it stays an orchestration document. | Cut to ~780. The 5h call sequence moved to `api-configuration.md`; the 23-item gate moved to a new `references/frontend/verify-gate.md` (now 25 items). No block exceeds 84 lines. |
...
| **Reference index points at directories; STEP 4 deferral reverted** | Reference reads are ~72% of a run's tool-result volume, so the index now names directories (`references/core/`, `packages/crud/`, `packages/workflow/`, `frontend/`) with a `README.md` in each routing to the right file. The agent uses those indexes as intended. **An attempt to also defer reading to each reference's point of use was reverted — it did not work.** Measured on a from-scratch build (`rx_manager`, 221 turns, $14.13) against the `wapp4` baseline (136 turns, $6.24): reference bytes read were 536KB vs 539KB, i.e. unchanged. Deferral moves *when* a doc enters context, not *how much* is read, and a later read sits on a larger prefix — so on a longer run it costs more, not less. Do not retry this without first reducing the volume actually read. The directory indexes and the file splits are kept: both are lossless and harmless. |
