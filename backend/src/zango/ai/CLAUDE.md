...
- **Bedrock model IDs are region-agnostic bare IDs** in `supported_models`. `_resolve_model_id()` prepends the geography prefix (`us.` / `eu.` / `apac.`) at call time for models with `requires_inference_profile=True`. IDs already starting with `us.` / `eu.` / `apac.` / `global.` pass through unchanged.
