...
from zango.test.client import BaseZangoRequestFactory, ZangoClient
...
class ZangoAppLoginTest(ZangoAppBaseTestCase):
    ...
    def get_test_module_path(cls):
        return os.path.join("apps/appauth/test_app_login")
    ...
    def setUpClass(cls):
        ...
        client.create_roles(
            tenant=cls.tenant, names=["app_login_user", "different_view_user"]
        )
    ...
    def test_app_login(self):
        ...
        session["role_id"] = self.app_user.roles.filter(
            name="app_login_user"
        ).values_list("id", flat=True)[0]
        session.save()

        logged_in = self.client.login(
            username="test_login_user@gmail.com", password="#Testpass123"
        )
    ...
    def test_logged_in_user_policy_map(self):
        ...
        new_role_ids = UserRoleModel.objects.filter(
            name__in=["app_login_user", "different_view_user"]
