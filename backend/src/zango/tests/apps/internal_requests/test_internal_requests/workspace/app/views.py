...
FORM_DATA_MULTIPART = {"field_1": "value_1", "field_2": "value_2"}


JSON_DATA = {"field_1": "value_1", "field_2": "value_2"}

GET_HEADERS = {"Accept": "text/plain"}

POST_HEADERS = {"Content-Type": "application/json"}

PUT_HEADERS = {"Cache-Control": "no-cache", "Content-Type": "application/json"}

DELETE_HEADERS = {"Authorization": "Bearer test-token"}
...
class TestAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "json_data":
            if (
                request.data.get("field_1") != JSON_DATA["field_1"]
                or request.data.get("field_2") != JSON_DATA["field_2"]
            ):
        ...
        if action == "file":
            ...
            if not uploaded_file:
                raise Exception("File not found in the request.")

            # Optionally, you can perform additional checks on the file, such as its content or size
            if uploaded_file.name != FILE_NAME:
                raise Exception(
                    f"Uploaded file name '{uploaded_file.name}' does not match the expected file name '{FILE_NAME}'."
                )
        ...
        if action == "multiple_files":
            ...
            if len(uploaded_files) != 2:
                raise Exception(
                    "Expected 2 files, but found {}".format(len(uploaded_files))
                )

        if action == "form_data_multipart":
            if (
                request.POST.get("field_1") != FORM_DATA_MULTIPART["field_1"]
                or request.POST.get("field_2") != FORM_DATA_MULTIPART["field_2"]
            ):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not action:
            raise Exception("Action not found")
        if action == "headers":
            if request.headers.get("Authorization") != DELETE_HEADERS.get(
                "Authorization"
            ):
...
class TestDataView(View):
    def validate_json_data(self, request):
        ...
        if (
            data.get("field_1") != JSON_DATA["field_1"]
            or data.get("field_2") != JSON_DATA["field_2"]
        ):
            return None, JsonResponse(
                {"error": "JSON data missing or incorrect"}, status=400
            )
...
class TestView(View):
    def post(self, request, *args, **kwargs):
        ...
        if action == "get_query_params":
            resp = requests.get(
                f"{url}/?{QUERY_PARAM}={PARAM_VALUE}&action=query_params"
            )
            return HttpResponse(resp.content)
        ...
        if action == "post_json_data":
            resp = requests.post(
                f"{url}?action=json_data", headers=POST_HEADERS, data=JSON_DATA
            )
            return HttpResponse(resp.content)

        if action == "put_json_data":
            resp = requests.put(
                f"{url}?action=json_data", headers=PUT_HEADERS, data=JSON_DATA
            )
            return HttpResponse(resp.content)
        ...
        if action == "post_file":
            resp = requests.post(
                f"{url}?action=file",
                files={FILE_NAME: open(os.path.join(FILE_DIR, FILE_NAME), "rb")},
                headers={
        ...
        if action == "post_form_data_multipart":
            resp = requests.post(
                f"{url}?action=form_data_multipart", data=FORM_DATA_MULTIPART
            )
            return HttpResponse(resp.content)
