...
def generate_models(path: str, no_of_models):
    with open(path, "a") as f:
        ...
        f.write(
            f"    author = models.ForeignKey(ModelAuthor, on_delete=models.CASCADE)\n"
        )
    ...
    with open(f"{tenant}/views.py", "w") as f:
        ...
        for i in range(100):
            ...
            for model_num in model_nums:
                f.write(
                    f"        mod = Model{model_num}.objects.create(field1='dota', field2=2)\n"
                )
...
def append_tenant_apps(tenants):
    ...
    print(tenants)
    with open("django_project/settings.py", "a") as f:
...
def append_urls(tenants):
    with open(f"django_project/urls_tenants.py", "w") as f:
...
def migration_operations(tenants):
    ...
    for tenant in tenants:
        subprocess.run(
            f"python manage.py tenant_command migrate --schema {tenant}", shell=True
        )
...
def create_tenants_and_domains(tenants):
    for index, tenant in enumerate(tenants):
        ten = TenantModel.objects.create(
            name=tenant, schema_name=tenant, description="desc"
        )
