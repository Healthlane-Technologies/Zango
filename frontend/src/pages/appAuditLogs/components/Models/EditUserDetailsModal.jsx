...
const EditUserDetailsForm = ({ closeModal }) => {
	...
	let initialValues = {
		name: appAuditLogsFormData?.name ?? '',
		email: appAuditLogsFormData?.email ?? '',
		mobile: appAuditLogsFormData?.mobile?.slice(4) ?? '',
		roles: appAuditLogsFormData?.roles?.map((eachApp) => eachApp.id) ?? [],
	};
					...
					>
						<div className="flex grow flex-col gap-[16px]">
							...
							<MultiSelectField
								...
								optionsData={appAuditLogsData?.dropdown_options?.roles ?? []}
								formik={formik}
							/>
						</div>
