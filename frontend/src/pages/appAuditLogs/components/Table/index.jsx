...
export default function Table({ tableData }) {
	...
	const platformAuditLogsData = useSelector(selectAppAuditLogsData);
	...
	const handleColumnSearch = (data) => {
		let tempTableData = JSON.parse(JSON.stringify(appAuditLogsTableData));
				...
				<div className="flex h-full items-start justify-start gap-[16px] border-b-[4px] border-[#F0F3F4] py-[12px] px-[20px] text-start">
					...
					<div className="translate-y-[-2px]">
						<TableDropdownFilter
							...
							placeholder="Select"
							value={
								find(appAuditLogsTableData?.columns, { id: 'object_type' })
									?.value
									? find(appAuditLogsTableData?.columns, {
											id: 'object_type',
									  })?.value
									: ''
							}
				...
				<div className="flex h-full items-start justify-start gap-[16px] border-b-[4px] border-[#F0F3F4] py-[12px] px-[20px] text-start">
					...
					<div className="">
						<TableDateRangeFilter
							...
							placeholder="Select"
							value={
								find(appAuditLogsTableData?.columns, { id: 'timestamp' })?.value
									? find(appAuditLogsTableData?.columns, {
											id: 'timestamp',
									  })?.value
									: ''
				...
				<div className="flex h-full items-start justify-start gap-[16px] border-b-[4px] border-[#F0F3F4] py-[12px] px-[20px] text-start">
					...
					<div className="translate-y-[-2px]">
						<TableDropdownFilter
							...
							placeholder="Select"
							value={
								find(appAuditLogsTableData?.columns, { id: 'action' })?.value
									? find(appAuditLogsTableData?.columns, {
											id: 'action',
									  })?.value
									: ''
	...
	const pagination = useMemo(
		() => ({
			pageIndex: appAuditLogsTableData?.pageIndex,
			pageSize: appAuditLogsTableData?.pageSize,
		}),
		[appAuditLogsTableData]
	);
	...
	const debounceSearch = debounce((data) => {
		dispatch(setAppAuditLogsTableData(data));
	}, 500);
	...
	useEffect(() => {
		searchRef.current.value = appAuditLogsTableData?.searchValue || '';
	}, []);
