...
const EditUserRolesDetailsForm = ({ closeModal }) => {
	...
	let initialValues = {
		...
		policies:
			...
			[],
		two_factor_auth: {
			required: appUserRolesFormData?.two_factor_auth?.required ?? false,
			allowedMethods: appUserRolesFormData?.two_factor_auth?.allowedMethods ?? ['email'],
		},
		password_policy: {
			min_length: appUserRolesFormData?.password_policy?.min_length ?? 8,
			require_numbers: appUserRolesFormData?.password_policy?.require_numbers ?? true,
			require_lowercase: appUserRolesFormData?.password_policy?.require_lowercase ?? true,
			require_uppercase: appUserRolesFormData?.password_policy?.require_uppercase ?? true,
			require_special_chars: appUserRolesFormData?.password_policy?.require_special_chars ?? true,
			password_expiry_days: appUserRolesFormData?.password_policy?.password_expiry_days ?? 90,
			password_history_count: appUserRolesFormData?.password_policy?.password_history_count ?? 3,
		},
	};
