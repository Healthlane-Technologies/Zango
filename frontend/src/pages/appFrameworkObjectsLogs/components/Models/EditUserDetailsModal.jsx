...
import {
	...
	selectAppFrameworkObjectsLogsFormData,
...
const EditUserDetailsForm = ({ closeModal }) => {
	...
	const appFrameworkObjectsLogsFormData = useSelector(
		selectAppFrameworkObjectsLogsFormData
	);

	const triggerApi = useApi();
	let initialValues = {
		name: appFrameworkObjectsLogsFormData?.name ?? '',
		email: appFrameworkObjectsLogsFormData?.email ?? '',
		mobile: appFrameworkObjectsLogsFormData?.mobile?.slice(4) ?? '',
		roles:
			appFrameworkObjectsLogsFormData?.roles?.map((eachApp) => eachApp.id) ??
			[],
	};
	...
	let onSubmit = (values) => {
		...
		const makeApiCall = async () => {
			const { response, success } = await triggerApi({
				url: `/api/v1/apps/${appId}/users/${appFrameworkObjectsLogsFormData?.id}/`,
				type: 'PUT',
				loader: true,
					...
					>
						<div className="flex grow flex-col gap-[16px]">
							...
							<MultiSelectField
								...
								optionsData={
									appFrameworkObjectsLogsData?.dropdown_options?.roles ?? []
								}
								formik={formik}
							/>
						</div>
