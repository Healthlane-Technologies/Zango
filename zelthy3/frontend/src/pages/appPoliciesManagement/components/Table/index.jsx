...
import { openIsViewPolicyModalOpen } from '../../slice';
import {
	PaginationState,
	createColumnHelper,
	flexRender,
...
export default function Table({ tableData }) {
	...
	const handleViewPolicyConfigure = () => {
		dispatch(openIsViewPolicyModalOpen());
	};

	const columnHelper = createColumnHelper();

	const columns = [
		columnHelper.accessor((row) => row.policy_name, {
			id: 'policy_name',
			header: () => (
		...
		columnHelper.accessor((row) => row.policy_id, {
			id: 'policy_id',
			header: () => (
		...
		columnHelper.accessor((row) => row.configuration, {
			...
			cell: (info) => (
				<div className="flex h-full flex-col border-b border-[#F0F3F4] py-[14px] px-[20px]">
					<button type="button" onClick={handleViewPolicyConfigure}>
		...
		columnHelper.accessor((row) => row.policy_description, {
