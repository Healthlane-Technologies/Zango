...
export default function Table({ tableData }) {
	...
	const columns = [
		columnHelper.accessor((row) => row.permission_name, {
			id: 'permission_name',
			header: () => (
		...
		columnHelper.accessor((row) => row.permission_id, {
			id: 'permission_id',
			header: () => (
		...
		columnHelper.accessor((row) => row.permission_type, {
			id: 'permission_type',
			header: () => (
				<div className="flex h-full items-start justify-start border-b-[4px] border-[#F0F3F4] py-[12px] px-[20px] text-start">
					<span className="min-w-max font-lato text-[11px] font-bold uppercase leading-[16px] tracking-[0.6px] text-[#6C747D]">
		...
		columnHelper.accessor((row) => row.permission_description, {
